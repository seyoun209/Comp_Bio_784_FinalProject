...
We excluded patients with missing tumor status and selected only patients when RSEM gene
expression was available. Of the 412 patients, 368 patients
...
unmethylated signals, respectively, and $\alpha$ represents an offset value. The given
...
$q_{i}$ denotes reads mapped to transcripts and $l_{i}$ is the transcript
...
1500bp upstream and 500bp downstream from the transcription start site (TSS).
A total of 21,010 genes (164,113 methylation probes) were tested. The below figure shows how the promoter regions were defined.
...
Functional enrichment analysis using ConsensusPathDb (Release35, CPDB:http://cpdb.molgen.mpg.de/CPDB)[5] was used to interpret the functional role of a gene identified with differential methylation status. Both significant pathway and gene ontology (GO) terms were defined as having an adjusted p-value ($\leq 0.05$). We constructed a protein-protein interaction (PPI) network for the significant methylated genes by string DB (Version 11.5).[6] We selected the interaction relationship between genes with the highest confidence $> 0.9$. Pathway and GO term were visualized using the `ggplot2` R package (version 3.4.0).
...
Out of the 20,118 genes, 128 were identified to have differential methylation by tumor status. Most genes (119) had a negative coefficient indicating hypomethylation. The figure below shows a volcano plot of the unadjusted p-values against estimated $\beta_j$ values, colored by significance. 
...
```{r, fig.cap='Figure 3a: Functional study of combined 134 genes'}
par(mfrow=c(1,2))
...
```{r, fig.cap='Figure 3b: Functional study of combined 134 genes'}
# plot: pathway
...
Using publicly available clinical and genetic tumor data from 368 patients with bladder urothelial cancer, we performed differential methylation analysis using limma and bumphunter. We identified differentially methylated genes by tumor status and performed functional enrichment analysis. Fifteen genes were identified, fourteen of which were protein-coding genes. Eleven genes were strongly related to bladder cancer and nine of these genes overlap with our limma results. We intersected this with RNA-seq expression data and identified 134 significant genes using ConsensusPathDB and revealed eight critical pathways, the most compelling being the VEGFA-VEGFR2 pathway. Using the STRING database, we constructed a protein-protein interaction (PPI) network and identified four genes functionally grouped into the VEGFA-VEGFR2  pathway. 
